...
const CustomTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="bg-[#1E293B] text-white px-3 py-2 rounded-lg shadow-lg text-sm">
      ...
      <p className="text-gray-300 mt-0.5">{formatCurrency(d.amount)} · {d.percentage.toFixed(1)}%</p>
    </div>
  );
};
...
const CategoryPieChart = ({ data, loading }) => {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      ...
      {chartData.length === 0 ? (
        <div className="h-56 flex items-center justify-center text-sm text-gray-400">No expenses this month</div>
      ) : (
        <div className="flex flex-col sm:flex-row items-center gap-6">
          <div className="w-52 h-52 flex-shrink-0 relative">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={chartData} cx="50%" cy="50%" innerRadius={60} outerRadius={95} paddingAngle={2} dataKey="amount" stroke="none">
